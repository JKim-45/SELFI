...
folder_name <- format(Sys.time(), "%y%m%d_%H%M_Healthy control and Late stage")
...
for(i in 1:n_of_exp)
{
  ...
  exclude_indices2 <- sample(1:n_of_samples_L,n_of_samples_L * r_of_exc)
  exclude_indices <- c(exclude_indices1, seq(51, 90), exclude_indices2 + n_of_samples_H + 40)
